...
{
    public void Register<T>(GenericInfo<T> layer, int index) where T : struct, ITimeStep;
}
...
public class TimeSyncModel : MonoBehaviour, ITimeSync
{
    ...
    /// <summary>
    /// 这里演示的是本地同步，如在网络框架下，需要将 T step传输到客户端，在客户端调用该方法。
    /// </summary>
    void UpdateStep<T>(T step, bool playback, int index) where T : struct, ITimeStep
    {
        var layer = layers[index] as GenericInfo<T>;
